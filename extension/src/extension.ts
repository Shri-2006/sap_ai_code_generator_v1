...
import { v4 as uuid } from "uuid"; // pulled in via npm install
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("sapCoder.openPanel", () => {
      openPanel(context);
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("sapCoder.run", async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("sapCoder.setModel", async () => {
      const config = vscode.workspace.getConfiguration("sapCoder");
      const backendUrl: string = config.get("backendUrl") ?? "http://localhost:3000";
...
async function writeFilesToWorkspace(files: { path: string; content: string }[]) {
...
async function gatherFileContext(): Promise<string> {
